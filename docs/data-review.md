...
- 自动生成 `id`
- 自动保存 `sourceUrl`
- 通过会员购公开详情接口提取标题、场馆、日期、封面、想去人数
- 默认只保留标题/场馆命中 `赛马娘`、`马娘`、`优俊少女`、`Uma Musume`、`umamusume`、`ウマ娘` 的活动
- 使用会员购场馆坐标并转换为百度坐标，点位会尽量落到场馆附近
- 不再采集活动简介，`description` 保持为空字符串
- 生成 `_reviewNotes`，告诉你哪些字段还需要人工确认

注意：脚本只读取会员购公开详情接口和公开 URL，不使用登录态、Cookie、验证码、签名绕过或非公开订单接口。

如果某个活动没有命中关键词但你仍然想保留，可以写：

```json
[
  {
    "url": "https://show.bilibili.com/platform/detail.html?id=1001421",
    "overrides": {
      "keep": true
    }
  }
